"""
Code for training the FCN8s without pretrained weights. Consists of two functions, train and val.
...
def train():
	for epoch in range(epochs):
		...
		if epoch%5==0:                                #Perform Validation every 5th epoch
			val(epoch)
		
		if epoch==2500 or epoch==5000:                #Save the model on every 2500th or 5000th epoch                
			model_path = os.path.join(model_dir,"epoch {}/".format(epoch))
...
def val(epoch):
	...
	for j in range(o.shape[1]):
		...
		tim.save('.../epoch_{}_layer_{}_target_img.png'.format(epoch,j)) #Fill ... with the folder where the images will be stored
